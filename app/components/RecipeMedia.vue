<template>
  <div class="w-full h-full">
    <!-- Video content -->
    <div v-if="video" class="w-full h-full">
      <video
        :key="`video-${videoKey}`"
    ...
    <div v-else class="w-full h-full bg-gradient-to-br" :class="placeholderClass">
      <div class="text-6xl opacity-20">{{ placeholderEmoji }}</div>
    </div>
  </div>
</template>
