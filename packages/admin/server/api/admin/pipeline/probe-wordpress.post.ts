 ...
 *
 * Query params:
 *   limit: number of publishers to probe (default 100, max 500)
 *   concurrency: parallel requests (default 5, max 20)
 */
...
export default defineEventHandler(async (event) => {
  ...
  const concurrency = Math.min(20, Math.max(1, Number(query.concurrency) || 5))
  ...
  return {
    ...
    notWordpress: completedCount - wordpressCount,
    failed: failedCount,
    newPluginsDiscovered: pluginMap.size - knownPlugins.length,
  }
})
