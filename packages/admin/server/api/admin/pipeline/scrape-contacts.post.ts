 ...
 *
 * Query params:
 *   limit: number of publishers to scrape (default 500, max 2000)
 *   concurrency: parallel requests (default 8, max 20)
 */
...
export default defineEventHandler(async (event) => {
  ...
  const concurrency = Math.min(20, Math.max(1, Number(query.concurrency) || 8))
