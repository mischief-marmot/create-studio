                        ...
                        <div class="px-4 py-8 flex flex-col space-y-8 overflow-y-auto">
                            ...
                            <div v-if="step.supply && step.supply.length > 0" class="box-gray">
                                <div class="flex justify-between cursor-pointer"
                                    @click="toggleStepIngredientsVisibility()">
                                    ...
                                    <li v-for="(supply, supplyIdx) in step.supply"
                                        ...
                                        <label class="flex items-start space-x-3 text-base-content">
                                            <input type="checkbox" class="checkbox checkbox-lg"
                                                :checked="isStepIngredientChecked(index, `${supply.name}`)"
                                                @change="toggleStepIngredient(index, `${supply.name}`)" />
...
// Shared storage manager will be initialized on client-side only
let sharedStorageManager: SharedStorageManager | null = null;

// Computed current recipe state
const currentRecipeState = computed(() => {
    return sharedStorageManager?.getCurrentRecipeState() || null;
});

// Timer manager will be initialized with storage manager
let timerManager: ReturnType<typeof useSharedTimerManager> | null = null;

// Storage helper functions
const toggleStepIngredientsVisibility = () => {
    if (sharedStorageManager) {
        sharedStorageManager.toggleStepIngredientsVisibility();
    }
};

const isStepIngredientChecked = (stepIndex: number, ingredientId: string): boolean => {
    return sharedStorageManager?.isStepIngredientChecked(stepIndex, ingredientId) || false;
};

const toggleStepIngredient = (stepIndex: number, ingredientId: string) => {
    if (sharedStorageManager) {
        sharedStorageManager.toggleStepIngredient(stepIndex, ingredientId);
    }
};
...
onMounted(async () => {
    ...
    // Initialize shared storage manager on client-side
    sharedStorageManager = new SharedStorageManager();
    
    // Initialize shared storage with domain and creation ID
    const domain = siteUrl ? normalizeDomain(siteUrl) : normalizeDomain(window.location.origin);
    const recipeKey = sharedStorageManager.initializeRecipe(domain, id);
    
    // Run migration from old storage to new shared storage
    sharedStorageManager.migrateFromLegacyStorage();

    // Initialize timer manager with storage manager
    timerManager = useSharedTimerManager(sharedStorageManager);
    provide('timerManager', timerManager);

    // Get state from shared storage
    const sharedState = sharedStorageManager.getCurrentRecipeState();
    if (sharedState) {
        // Use shared storage state
        currentSlide.value = sharedState.currentStep;
        imageHeight.value = sharedState.imageHeight;
        isImageCollapsed.value = sharedState.isImageCollapsed;
    }

    // Restore timers from store
    timerManager.restoreTimersFromStore();

    // Set up watchers to update shared storage
    watch(currentSlide, (newSlide) => {
        if (sharedStorageManager) {
            sharedStorageManager.setCurrentStep(newSlide);
        }
    });

    watch([imageHeight, isImageCollapsed], ([height, collapsed]) => {
        if (sharedStorageManager) {
            sharedStorageManager.setImageState(height, collapsed);
        }
    });
